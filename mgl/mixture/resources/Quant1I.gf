...
--1 quant1
lin
	exist vars set prop 	 = mkFullPropPlus (existential (andVars vars) set.v prop.v) ;				--(L.exist vars.s3 set.s prop.s) False;

	forall vars prop 	 = mkFullPropPlus (forall_quantifier (andVars vars) prop.v) ; 				--(L.forall vars.s3 prop.s) False ;

	forall_set vars set prop = mkFullPropPlus (forall_quantifier (adverbNP (inObj set.v) (andVars vars)) prop.v) ; 	-- (L.forall_set vars.s3 set.s prop.s) False ;
}
