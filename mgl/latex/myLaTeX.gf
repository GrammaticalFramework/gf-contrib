...
--		\f,x -> mkPrec highest (sFun1 f x.s) ;
--		\f,x -> mkPrec highest (sFun1 f (usePrec highest x)) ;

	ufun1 : Str -> TermPrec -> TermPrec = 
		\f,x -> mkPrec highest (uFun1 f (usePrec 4 x)) ;
--		\f,x -> mkPrec highest (uFun1 f x.s) ;
--		\f,x -> mkPrec highest (uFun1 f (usePrec highest x)) ;

	funOpt1 : Str -> Str -> TermPrec -> TermPrec =
		\f,opt,x -> mkPrec highest ("\\" + f + "[" ++ opt ++ "]{" ++ x.s ++ "}") ;
--		\f,opt,x -> mkPrec highest ("\\" + f + "[" ++ opt ++ "]{" ++ (usePrec highest x) ++ "}") ;
