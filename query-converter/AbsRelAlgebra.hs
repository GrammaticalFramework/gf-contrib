...
module AbsRelAlgebra (Tree(..), Rels, Rel, Cond, Exp, Projection, Renaming, Aggregation, Function, Ident, johnMajorEq, module ComposOp) where
...
data Tag = Rels_ | Rel_ | Cond_ | Exp_ | Projection_ | Renaming_ | Aggregation_ | Function_ | Ident_
...
data Tree :: Tag -> * where
    ...
    EAggr :: Function -> Ident -> Tree Exp_
    ...
    AApp :: Function -> Ident -> Tree Aggregation_
    ARename :: Function -> Ident -> Exp -> Tree Aggregation_
    FAvg :: Tree Function_
    FSum :: Tree Function_
    FMax :: Tree Function_
    FMin :: Tree Function_
...
instance Compos Tree where
  compos r a f t = case t of
      ...
      RSort exps rel -> r RSort `a` foldr (a . a (r (:)) . f) (r []) exps `a` f rel
      RDistinct rel -> r RDistinct `a` f rel
      RUnion rel0 rel1 -> r RUnion `a` f rel0 `a` f rel1
      RJoin rel0 rel1 -> r RJoin `a` f rel0 `a` f rel1
      ...
      EAggr function i -> r EAggr `a` f function `a` f i
      ...
      AApp function i -> r AApp `a` f function `a` f i
      ARename function i exp -> r ARename `a` f function `a` f i `a` f exp
      _ -> r t
...
instance Show (Tree c) where
  showsPrec n t = case t of
    ...
    RSort exps rel -> opar n . showString "RSort" . showChar ' ' . showsPrec 1 exps . showChar ' ' . showsPrec 1 rel . cpar n
    ...
    RJoin rel0 rel1 -> opar n . showString "RJoin" . showChar ' ' . showsPrec 1 rel0 . showChar ' ' . showsPrec 1 rel1 . cpar n
    ...
    EAggr function i -> opar n . showString "EAggr" . showChar ' ' . showsPrec 1 function . showChar ' ' . showsPrec 1 i . cpar n
    ...
    AApp function i -> opar n . showString "AApp" . showChar ' ' . showsPrec 1 function . showChar ' ' . showsPrec 1 i . cpar n
    ARename function i exp -> opar n . showString "ARename" . showChar ' ' . showsPrec 1 function . showChar ' ' . showsPrec 1 i . showChar ' ' . showsPrec 1 exp . cpar n
...
johnMajorEq (RSort exps rel) (RSort exps_ rel_) = exps == exps_ && rel == rel_
johnMajorEq (RDistinct rel) (RDistinct rel_) = rel == rel_
johnMajorEq (RUnion rel0 rel1) (RUnion rel0_ rel1_) = rel0 == rel0_ && rel1 == rel1_
johnMajorEq (RJoin rel0 rel1) (RJoin rel0_ rel1_) = rel0 == rel0_ && rel1 == rel1_
...
johnMajorEq (EAggr function i) (EAggr function_ i_) = function == function_ && i == i_
...
johnMajorEq (AApp function i) (AApp function_ i_) = function == function_ && i == i_
johnMajorEq (ARename function i exp) (ARename function_ i_ exp_) = function == function_ && i == i_ && exp == exp_
...
index (RUnion _ _) = 8
index (RJoin _ _) = 9
index (RThetaJoin _ _ _) = 10
index (RIntersect _ _) = 11
index (RCartesian _ _) = 12
index (RExcept _ _) = 13
index (CEq _ _) = 14
index (CNEq _ _) = 15
index (CLt _ _) = 16
index (CGt _ _) = 17
...
index (ESub _ _) = 34
index (PExp _) = 35
index (PRename _ _) = 36
index (RRelation _) = 37
index (RAttributes _ _) = 38
index (AApp _ _) = 39
index (ARename _ _ _) = 40
index (FAvg ) = 41
index (FSum ) = 42
index (FMax ) = 43
index (FMin ) = 44
...
compareSame (RSort exps rel) (RSort exps_ rel_) = mappend (compare exps exps_) (compare rel rel_)
...
compareSame (RJoin rel0 rel1) (RJoin rel0_ rel1_) = mappend (compare rel0 rel0_) (compare rel1 rel1_)
...
compareSame (EAggr function i) (EAggr function_ i_) = mappend (compare function function_) (compare i i_)
...
compareSame (AApp function i) (AApp function_ i_) = mappend (compare function function_) (compare i i_)
compareSame (ARename function i exp) (ARename function_ i_ exp_) = mappend (compare function function_) (mappend (compare i i_) (compare exp exp_))
