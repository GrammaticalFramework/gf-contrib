...
instance Print Statement where
  prt i e = case e of
    ...
    SDelete id where_ -> prPrec i 0 (concatD [doc (showString "DELETE"), doc (showString "FROM"), prt 0 id, prt 0 where_])
    SUpdate id settings where_ -> prPrec i 0 (concatD [doc (showString "UPDATE"), prt 0 id, doc (showString "SET"), prt 0 settings, prt 0 where_])
    ...
    SCreateTrigger id1 triggertime triggeractions id2 triggereach triggerbody -> prPrec i 0 (concatD [doc (showString "CREATE"), doc (showString "TRIGGER"), prt 0 id1, prt 0 triggertime, prt 0 triggeractions, doc (showString "ON"), prt 0 id2, doc (showString "FOR"), doc (showString "EACH"), prt 0 triggereach, prt 0 triggerbody])
    SQuery query -> prPrec i 0 (concatD [prt 0 query])
  prtList _ [] = (concatD [])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ";"), prt 0 xs])
instance Print Query where
  prt i e = case e of
    QSelect distinct columns tables where_ group having order -> prPrec i 2 (concatD [doc (showString "SELECT"), prt 0 distinct, prt 0 columns, doc (showString "FROM"), prt 0 tables, prt 0 where_, prt 0 group, prt 0 having, prt 0 order])
    QSetOperation query1 setoperation all query2 -> prPrec i 1 (concatD [prt 1 query1, prt 0 setoperation, prt 0 all, prt 2 query2])
...
instance Print Table where
  prt i e = case e of
    ...
    TJoin table1 jointype table2 joinon -> prPrec i 0 (concatD [prt 0 table1, prt 0 jointype, doc (showString "JOIN"), prt 1 table2, prt 0 joinon])
    TNaturalJoin table1 jointype table2 -> prPrec i 0 (concatD [prt 0 table1, doc (showString "NATURAL"), prt 0 jointype, doc (showString "JOIN"), prt 1 table2])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ","), prt 0 xs])
...
instance Print Condition where
  prt i e = case e of
    ...
    CBetween exp1 not exp2 exp3 -> prPrec i 2 (concatD [prt 0 exp1, prt 0 not, doc (showString "BETWEEN"), prt 0 exp2, doc (showString "AND"), prt 0 exp3])
    CIn exp not values -> prPrec i 2 (concatD [prt 0 exp, prt 0 not, doc (showString "IN"), prt 0 values])
    CAnd condition1 condition2 -> prPrec i 1 (concatD [prt 1 condition1, doc (showString "AND"), prt 2 condition2])
    COr condition1 condition2 -> prPrec i 0 (concatD [prt 0 condition1, doc (showString "OR"), prt 1 condition2])
...
instance Print Exp where
  prt i e = case e of
    ...
    EQual id1 id2 -> prPrec i 4 (concatD [prt 0 id1, doc (showString "."), prt 0 id2])
...
instance Print Typing where
  prt i e = case e of
    TColumn id type_ inlineconstraints -> prPrec i 0 (concatD [prt 0 id, prt 0 type_, prt 0 inlineconstraints])
...
instance Print InlineConstraint where
  prt i e = case e of
    ...
    ICReferences id1 id2 policys -> prPrec i 0 (concatD [doc (showString "REFERENCES"), prt 0 id1, doc (showString "("), prt 0 id2, doc (showString ")"), prt 0 policys])
...
instance Print Constraint where
  prt i e = case e of
    ...
    CReferences ids1 id ids2 policys -> prPrec i 0 (concatD [doc (showString "FOREIGN"), doc (showString "KEY"), doc (showString "("), prt 0 ids1, doc (showString ")"), doc (showString "REFERENCES"), prt 0 id, doc (showString "("), prt 0 ids2, doc (showString ")"), prt 0 policys])
...
instance Print Alteration where
  prt i e = case e of
    ...
    AAlter id type_ -> prPrec i 0 (concatD [doc (showString "MODIFY"), doc (showString "COLUMN"), prt 0 id, prt 0 type_])
...
instance Print TriggerAction where
  ...
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString "OR"), prt 0 xs])
